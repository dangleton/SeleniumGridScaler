...
public class AutomationTestRunServlet extends RegistryBasedServlet implements RegistryRetriever {
    ...
    private void initCleanupThreads() {
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(
                new AutomationPendingNodeRegistryTask(this, ec2),
                60L, 15L, TimeUnit.SECONDS);
...
//                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Test run already exists with the same UUID.");
     ...
     * @return
     */
    public static List<AutomationDynamicNode> startNodes(VmManager ec2, String uuid, int threadCountRequested,
            String browser, Platform platform) throws NodesCouldNotBeStartedException {
        ...
        try {
            ...
            for (Instance instance : instances) {
                AutomationDynamicNode createdNode = new AutomationDynamicNode(uuid, instance.getInstanceId(), browser,
                        platform, instance.getPrivateIpAddress(), startDate, numThreadsPerMachine,
                        instance.getInstanceType());
                // Add the node as pending startup to our context so we can track it in
