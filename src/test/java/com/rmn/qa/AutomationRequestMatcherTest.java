...
public class AutomationRequestMatcherTest extends BaseTest {
    ...
    public void testRequestMatchingBrowsers() throws IOException, ServletException {
        ...
        AutomationContext.getContext().setTotalNodeCount(50);
        int freeThreads = new AutomationRequestMatcher().getNumFreeThreadsForParameters(proxySet,
                new AutomationRunRequest(browser));
    ...
    public void testRequestNewRunNotStartedDifferentBrowser() throws IOException, ServletException {
        ...
        TestSlot nonMatchingTestSlot =
                new TestSlot(nonMatchingProxy, SeleniumProtocol.WebDriver, null, nonMatchingCapabilities);
        ...
        TestSlot matchingTestSlot =
                new TestSlot(nonMatchingProxy, SeleniumProtocol.WebDriver, null, matchingCapabilities);
        matchingProxy.setMultipleTestSlots(matchingTestSlot, 10);

        AutomationContext.getContext().setTotalNodeCount(50);
        int freeThreads = new AutomationRequestMatcher().getNumFreeThreadsForParameters(proxySet,
                new AutomationRunRequest(matchingBrowser));
        Assert.assertEquals("Nodes should be free even though run is in progress as browsers do not match", 10,
                freeThreads);
    }
    ...
    public void testRequestOldRunInProgress() throws IOException, ServletException {
        ...
        AutomationContext.getContext().addNode(node);
        String runId = "runId";
        AutomationContext.getContext().addRun(new AutomationRunRequest(runId, 10, "firefox", null, null,
                AutomationUtils.modifyDate(new Date(), -2, Calendar.MINUTE)));
        ProxySet proxySet = new ProxySet(false);
        MockRemoteProxy proxy = new MockRemoteProxy();
        proxy.setMaxNumberOfConcurrentTestSessions(50);
        proxy.setCapabilityMatcher(new AutomationCapabilityMatcher());
        ...
        int freeThreads = new AutomationRequestMatcher().getNumFreeThreadsForParameters(proxySet,
                new AutomationRunRequest(browser));
        Assert.assertEquals("Nodes should be considered free since run is considered old at this point ", 5,
                freeThreads);
    }
    ...
    public void testNew2() throws IOException, ServletException {
        ...
        config2.custom.put(AutomationConstants.INSTANCE_ID, nodeId);
