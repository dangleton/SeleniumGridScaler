...
public class AutomationPendingNodeRegistryTaskTest extends BaseTest {
    ...
    public void testPendingNodeRemoved() {
        ...
        Assert.assertTrue("Node should be pending before task runs",
                AutomationContext.getContext().pendingNodeExists(instanceId));
        task.doWork();
        Assert.assertFalse("Instance should not show as pending after task has run",
                AutomationContext.getContext().pendingNodeExists(instanceId));
    }
    ...
    public void testPendingNodeNotRemovedMismatchInstanceId() {
        ...
        Assert.assertTrue("Node should be pending before task runs",
                AutomationContext.getContext().pendingNodeExists(instanceIdDifferent));
        task.doWork();
        Assert.assertTrue("Instance should still be pending as it hasn't come online",
                AutomationContext.getContext().pendingNodeExists(instanceIdDifferent));
        Assert.assertFalse("Instance should not be pending as it never was pending to begin with",
                AutomationContext.getContext().pendingNodeExists(instanceId));
    ...
    public void testPendingNodeNotRemovedNoConfigId() {
        ...
        Assert.assertTrue("Node should be pending before task runs",
                AutomationContext.getContext().pendingNodeExists(instanceId));
        task.doWork();
        Assert.assertTrue("Node should still be pending after task runs as there was no instance id in the config",
                AutomationContext.getContext().pendingNodeExists(instanceId));
    }
